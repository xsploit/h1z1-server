...
class CompileNavigationTransitionsTests(unittest.TestCase):
    ...
    def test_committed_world_transition_artifact_matches_provenance(self):
        ...
        self.assertEqual(len(compiled), 283)
        self.assertEqual(
        ...
        for actor, count in (
            ("Common_Structures_StoreFront01.adr", 36),
            ("Common_Structures_StoreFront02.adr", 38),
            ("Common_Structures_StoreFront03.adr", 28),
        ):
            with self.subTest(actor=actor):
                self.assertEqual(
                    sum(
                        entry["name"].startswith(f"{actor} #")
                        for entry in compiled
                    ),
                    count,
                )
        self.assertEqual(
            sum(
                entry["name"].startswith(
                    "Common_Structures_HardwareStore01.adr #"
                )
                for entry in compiled
            ),
            4,
