...
function verifyRuntime(runtimeRoot, manifest) {
  if (
    ...
    throw new Error(
      "[NAV] rejected the experimental navigation64 runtime manifest"
    );
  }
  ...
  if (artifactId !== expectedArtifactId) {
    throw new Error(
      "[NAV] rejected the experimental runtime artifact identity"
    );
  }
  ...
  if (
    ...
    throw new Error("[NAV] rejected the experimental runtime file closure");
  }
  ...
  for (const file of manifest.files) {
    ...
    if (
      ...
      throw new Error(`[NAV] rejected runtime file identity: ${file.path}`);
    }
  }
}

module.exports = function configureExperimentalNavigation64(options = {}) {
  const environment = options.environment || process.env;
  const quickStartRoot = options.quickStartRoot || __dirname;
  environment.NAV_MONOLITHIC_64 ??= "1";
  ...
  for (const requiredPath of [manifestPath, coreModule, wasmModule]) {
    if (!fs.existsSync(requiredPath)) {
      throw new Error(
        `[NAV] experimental navigation64 file missing: ${requiredPath}`
      );
    }
  }
  ...
  environment.NAV_STREAMING_MUTATION = "0";
  return {
    enabled: true,
    mode: "monolithic64",
